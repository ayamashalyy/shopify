...
class SearchViewModel {
                ...
                if let products = products {
                    ...
                    if Authorize.isRegistedCustomer() {
                        self.checkIsFav(productFromApi: products) { productWithFavStatus in
                            self.recevingProductFromANotherScreen = productWithFavStatus
                        }
                    } else {
                        self.recevingProductFromANotherScreen = products
                    }
    ...
    func filterProducts(filteredProducts: [Product], byPrice price: Float) -> [Product] {
        return filteredProducts.filter { product in
            if let productPrice = Float(product.variants.first?.price ?? "") {
